...
def neighbor_dif_batch(f1, f2):
    ...
    . 1 .
    """
    pad = lambda t, p: torch.nn.functional.pad(t[None], p, mode='replicate')[0]
...
def neighbor_consistency_batch(u, u_ref, v, v_ref):
    u = torch.stack(neighbor_dif_batch(u, u_ref))
    del u_ref
    v = torch.stack(neighbor_dif_batch(v, v_ref))
    del v_ref
    return consistency(u) + consistency(v)


def loss_function_batch(u, v, u0, v0, fidelity_w=0.4):
    """
    u, v    — batch of initial values    (angles)
    u0, v0  — batch of optimized values  (angles)
    """
    # fidelity
    fidelity = consistency(u - u0.data) + consistency(v - v0.data)
    del u0, v0
    ...
    # self-consistency
    uuvv = neighbor_consistency_batch(u, u.data, v, v.data)
    uvvu = neighbor_consistency_batch(u, v.data, v, u.data)
    sc = torch.min(uuvv, uvvu)
    del uuvv, uvvu
    sc = sc.sum(dim=0).mean()
